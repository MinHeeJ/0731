...
ON CONFLICT (organization_code) DO UPDATE SET organization_name = EXCLUDED.organization_name, updated_at = now();
...
ON CONFLICT (relation_id) DO UPDATE SET updated_at = now();
...
ON CONFLICT (login_id) DO UPDATE SET password_hash = EXCLUDED.password_hash, system_use_enabled = true, status = 'ACTIVE', updated_at = now();
...
ON CONFLICT (role_code) DO UPDATE SET role_name = EXCLUDED.role_name, purpose = EXCLUDED.purpose, updated_at = now();
...
ON CONFLICT (assignment_id) DO UPDATE SET status = 'ACTIVE', updated_at = now();
...
ON CONFLICT (menu_id) DO UPDATE SET menu_name = EXCLUDED.menu_name, url_path = EXCLUDED.url_path, updated_at = now();
...
ON CONFLICT (target_type, target_id, menu_id) DO UPDATE SET allowed = true, updated_at = now();
...
ON CONFLICT (group_id, code_value) DO UPDATE SET code_name = EXCLUDED.code_name, updated_at = now();
